// Данные каталога и пула ключей из ТЗ (Приложение. Материалы)
...
export function generateSyntheticProducts(count = 10000) {
  ...
  for (let i = 0; i < count; i++) {
    products.push({
      sku: `SKU-SYN-${String(i).padStart(6, '0')}`,
...
export function splitPoolBetweenSuppliers(keys = KEYS, skus = PRODUCTS.map((p) => p.sku)) {
  // key -> sku (циклически)
  const bySku = new Map();
  keys.forEach((code, i) => {
    const sku = skus[i % skus.length];
    if (!bySku.has(sku)) bySku.set(sku, []);
    bySku.get(sku).push(code);
  });
  // делим каждый sku-список пополам: чётные — A, нечётные — B
  const a = new Map();
  const b = new Map();
  for (const [sku, codes] of bySku) {
    const half = codes.filter((_, i) => i % 2 === 0);
    const halfB = codes.filter((_, i) => i % 2 === 1);
    if (half.length) a.set(sku, half);
    if (halfB.length) b.set(sku, halfB);
  }
  return { a, b };
}
